...
function! paredit#InitBuffer()
    ...
    if g:paredit_mode
        ...
        inoremap <buffer> <expr>   (            paredit#InsertOpening('(',')')
        inoremap <buffer> <silent> )            <C-R>=(pumvisible() ? "\<lt>C-Y>" : "")<CR><C-O>:let save_ve=&ve<CR><C-O>:set ve=onemore<CR><C-O>:<C-U>call paredit#InsertClosing('(',')')<CR><C-O>:let &ve=save_ve<CR>
        inoremap <buffer> <expr>   "            paredit#InsertQuotes()
        inoremap <buffer> <expr>   <BS>         paredit#Backspace(0)
        inoremap <buffer> <expr>   <Del>        paredit#Del()
        nnoremap <buffer> <silent> (            :<C-U>call paredit#FindOpening('(',')',0)<CR>
        nnoremap <buffer> <silent> )            :<C-U>call paredit#FindClosing('(',')',0)<CR>
        vnoremap <buffer> <silent> (            <Esc>:<C-U>call paredit#FindOpening('(',')',1)<CR>
        vnoremap <buffer> <silent> )            <Esc>:<C-U>call paredit#FindClosing('(',')',1)<CR>
        nnoremap <buffer> <silent> [[           :<C-U>call paredit#FindDefunBck()<CR>
        nnoremap <buffer> <silent> ]]           :<C-U>call paredit#FindDefunFwd()<CR>
        ...
        vnoremap <buffer> <silent> d            :<C-U>call paredit#Delete(visualmode(),1)<CR>
        vnoremap <buffer> <silent> x            :<C-U>call paredit#Delete(visualmode(),1)<CR>
        vnoremap <buffer> <silent> <Del>        :<C-U>call paredit#Delete(visualmode(),1)<CR>
        nnoremap <buffer> <silent> c            :set opfunc=paredit#Change<CR>g@
        vnoremap <buffer> <silent> c            :<C-U>call paredit#Change(visualmode(),1)<CR>
        nnoremap <buffer> <silent> dd           :<C-U>call paredit#DeleteLines()<CR>
        nnoremap <buffer> <silent> cc           :<C-U>call paredit#ChangeLines()<CR>
        nnoremap <buffer> <silent> cw           :<C-U>call paredit#ChangeSpec('cw',1)<CR>
        nnoremap <buffer> <silent> cb           :<C-U>call paredit#ChangeSpec('cb',0)<CR>
        nnoremap <buffer> <silent> ciw          :<C-U>call paredit#ChangeSpec('ciw',1)<CR>
        ...
        execute 'nmap     <buffer> <silent> ' . g:paredit_leader.'I   :<C-U>call paredit#Raise()<CR>'
        if &ft == 'clojure'
            inoremap <buffer> <expr>   [            paredit#InsertOpening('[',']')
            inoremap <buffer> <silent> ]            <C-R>=(pumvisible() ? "\<lt>C-Y>" : "")<CR><C-O>:let save_ve=&ve<CR><C-O>:set ve=onemore<CR><C-O>:<C-U>call paredit#InsertClosing('[',']')<CR><C-O>:let &ve=save_ve<CR>
            inoremap <buffer> <expr>   {            paredit#InsertOpening('{','}')
        ...
        if g:paredit_shortmaps
            ...
            nnoremap <buffer> <silent> <            :<C-U>call paredit#MoveLeft()<CR>
            nnoremap <buffer> <silent> >            :<C-U>call paredit#MoveRight()<CR>
            nnoremap <buffer> <silent> O            :<C-U>call paredit#Split()<CR>
            nnoremap <buffer> <silent> J            :<C-U>call paredit#Join()<CR>
            nnoremap <buffer> <silent> W            :<C-U>call paredit#Wrap('(',')')<CR>
            vnoremap <buffer> <silent> W            :<C-U>call paredit#WrapSelection('(',')')<CR>
            nnoremap <buffer> <silent> S            :<C-U>call paredit#Splice()<CR>
            ...
            nnoremap <buffer> <silent> S            V:<C-U>call paredit#Change(visualmode(),1)<CR>
            execute 'nnoremap <buffer> <silent> ' . g:paredit_leader.'<  :<C-U>call paredit#MoveLeft()<CR>'
            execute 'nnoremap <buffer> <silent> ' . g:paredit_leader.'>  :<C-U>call paredit#MoveRight()<CR>'
            execute 'nnoremap <buffer> <silent> ' . g:paredit_leader.'O  :<C-U>call paredit#Split()<CR>'
            execute 'nnoremap <buffer> <silent> ' . g:paredit_leader.'J  :<C-U>call paredit#Join()<CR>'
            execute 'nnoremap <buffer> <silent> ' . g:paredit_leader.'W  :<C-U>call paredit#Wrap("(",")")<CR>'
            execute 'vnoremap <buffer> <silent> ' . g:paredit_leader.'W  :<C-U>call paredit#WrapSelection("(",")")<CR>'
            execute 'nnoremap <buffer> <silent> ' . g:paredit_leader.'S  :<C-U>call paredit#Splice()<CR>'
        endif

        if g:paredit_electric_return && mapcheck( "<CR>", "i" ) == ""
            " Do not override any possible mapping for <Enter>
            inoremap <buffer> <expr>   <CR>         paredit#Enter()
        endif
    else
        ...
        if mapcheck( "<CR>", "i" ) == "paredit#Enter()"
...
function! paredit#Opfunc( func, type, visualmode )
...
function! paredit#SetDelete( count )
    let s:repeat = a:count
    set opfunc=paredit#Delete
endfunction

" General delete operator handling
function! paredit#Delete( type, ... )
    call paredit#Opfunc( 'd', a:type, a:0 )
    if s:repeat > 1
...
function! paredit#Change( type, ... )
    let ve_save = &virtualedit
    set virtualedit=all
    call paredit#Opfunc( 'c', a:type, a:0 )
    startinsert
    let &virtualedit = ve_save
endfunction

" Delete v:count number of lines
function! paredit#DeleteLines()
    if v:count > 1
        silent exe "normal! V" . (v:count-1) . "j\<Esc>"
    else
        silent exe "normal! V\<Esc>"
    endif
    call paredit#Delete(visualmode(),1)
endfunction

" Change v:count number of lines
function! paredit#ChangeLines()
    if v:count > 1
        silent exe "normal! V" . (v:count-1) . "j\<Esc>"
    else
        silent exe "normal! V\<Esc>"
    endif
    call paredit#Change(visualmode(),1)
endfunction
...
function! paredit#ChangeSpec( cmd, dir )
    let line = getline( '.' )
    if a:dir == 0
        " Changing backwards
        let c =  col( '.' ) - 2
...
function! paredit#FindOpening( open, close, select )
...
function! paredit#FindClosing( open, close, select )
    let open  = escape( a:open , '[]' )
    let close = escape( a:close, '[]' )
    if a:select
        let line = getline( '.' )
...
function! paredit#InsertOpening( open, close )
...
function! paredit#InsertClosing( open, close )
    if !g:paredit_mode || s:InsideComment() || s:InsideString() || !s:IsBalanced()
...
" Handle <BS> keypress
function! paredit#Backspace( repl_mode )
...
" Wrap current visual block in parens of the given kind
" Keep visual mode
function! paredit#WrapSelection( open, close )
...
function! paredit#Wrap( open, close )
